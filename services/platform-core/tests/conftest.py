...
def get_settings_override() -> PlatformCoreSettings:
    ...
    settings.DB.DATABASE_URL = os.getenv(
        "DB__DATABASE_URL", settings.DB.DATABASE_URL
    )
...
def prevent_real_redis_calls(monkeypatch):
    ...
    # Apply the patches
    monkeypatch.setattr(
        "app.db.redis.initialize_redis_pool", mock_initialize_redis_pool
    )
...
async def override_get_redis(
    app: FastAPI, fake_redis_client: fakeredis.aioredis.FakeRedis
):
    """
...
async def settings_override() -> PlatformCoreSettings:
    ...
    # This URL doesn't matter, we'll patch Redis
    settings.CACHE.REDIS_URL = "redis://fake:6379/1"
    # Use SQLite file-based database for testing to ensure all connections
