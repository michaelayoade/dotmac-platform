...
async def create_webhook_endpoint(
    endpoint: WebhookEndpointCreate, db: AsyncSession = Depends(get_db)
):
    """
...
@router.delete(
    "/endpoints/{endpoint_id}", status_code=http_status.HTTP_204_NO_CONTENT
)
...
@router.post(
    "/endpoints/{endpoint_id}/test", response_model=WebhookDeliveryResponse
)
...
@router.get(
    "/deliveries/{delivery_id}", response_model=WebhookDeliveryResponse
)
...
@router.post(
    "/trigger/{event_type}", status_code=http_status.HTTP_202_ACCEPTED
)
