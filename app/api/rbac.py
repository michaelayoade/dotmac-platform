...
@router.delete(
    "/roles/{role_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_role("admin"))]
)
...
@router.patch(
    "/permissions/{permission_id}", response_model=PermissionRead, dependencies=[Depends(require_role("admin"))]
)
...
@router.patch(
    "/role-permissions/{link_id}", response_model=RolePermissionRead, dependencies=[Depends(require_role("admin"))]
)
...
@router.delete(
    "/role-permissions/{link_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_role("admin"))]
)
...
@router.delete(
    "/person-roles/{link_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_role("admin"))]
)
