...
def list_channels(
    ...
    result = []
    for ch in channels:
        serialized = svc.serialize_channel(ch, config_masked=svc.mask_config(ch))
        result.append(NotificationChannelRead.model_validate(serialized))
    return result
...
def create_channel(
    ...
    try:
        ...
        db.commit()
        serialized = svc.serialize_channel(channel, config_masked=svc.mask_config(channel))
        return NotificationChannelRead.model_validate(serialized)
