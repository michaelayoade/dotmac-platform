...
class InstanceService:
    ...
    def create_with_catalog(
        ...
        *,
        ...
        Returns (instance, deployment_id). deployment_id is set when
        admin_password is provided, indicating the caller should kick
        the deploy task after committing.
        """
        git_repo_id, git_ref = self.resolve_catalog_repo(catalog_item_id)
        ...
        # Apply version pin from catalog release
        if git_ref:
            instance.git_tag = git_ref
            self.db.flush()

        # Auto-deploy when admin password is provided
    ...
    def delete(self, instance_id: UUID) -> None:
        ...
        # Manually clean dependent rows because many FKs to instances do not
        # use DB-level ON DELETE CASCADE.
        for table in reversed(Base.metadata.sorted_tables):
            if table is instance_table:
                continue

            fk_cols = []
            for col in table.columns:
                for fk in col.foreign_keys:
                    if fk.column.table is instance_table and fk.column.name == "instance_id":
                        fk_cols.append(col)
                        break
            ...
            nullable_cols = [col for col in fk_cols if col.nullable]
            required_cols = [col for col in fk_cols if not col.nullable]

            for col in nullable_cols:
                self.db.execute(update(table).where(col == instance_id).values({col.name: None}))

            if required_cols:
                self.db.execute(delete(table).where(or_(*[col == instance_id for col in required_cols])))
    ...
    def generate_env(
        ...
        is_https = (instance.app_url or "").startswith("https://")
        cookie_secure = "true" if is_https else "false"
        cookie_samesite = "strict" if is_https else "lax"
    ...
    def generate_bootstrap_db_script(self) -> str:
        ...
        return textwrap.dedent('''\
            ...
            def main():
                ...
                # Empty values in .env should still use sane defaults for first bootstrap.
                sector_type = (os.environ.get("BOOTSTRAP_SECTOR_TYPE", "PRIVATE") or "PRIVATE").strip().upper()
                framework = (os.environ.get("BOOTSTRAP_FRAMEWORK", "IFRS") or "IFRS").strip().upper()
                currency = (os.environ.get("BOOTSTRAP_CURRENCY", "NGN") or "NGN").strip().upper()
