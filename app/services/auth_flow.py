...
def _refresh_cookie_path(db: Session | None) -> str:
    return _env_value("REFRESH_COOKIE_PATH") or _setting_value(db, "refresh_cookie_path") or "/"
...
def _load_rbac_claims(db: Session, person_id: str):
    ...
    roles = list(db.scalars(roles_stmt).all())
    permissions = list(db.scalars(perms_stmt).all())
...
class AuthFlow(ListResponseMixin):
    ...
    def mfa_verify(db: Session, mfa_token: str, code: str, request: Request):
        ...
        totp = pyotp.TOTP(secret)
        now = _now()
        last_used = _as_utc(method.last_used_at)
        if last_used and totp.timecode(last_used) == totp.timecode(now):
            raise HTTPException(status_code=401, detail="MFA code already used")
        if not totp.verify(code, valid_window=0):
            raise HTTPException(status_code=401, detail="Invalid MFA code")
    ...
    def refresh(db: Session, refresh_token: str, request: Request):
        ...
        if not session:
            reused = db.scalar(select(SessionRefreshToken).where(SessionRefreshToken.token_hash == token_hash))
            if reused:
                reused_session = db.get(AuthSession, reused.session_id)
                if reused_session and reused_session.status == SessionStatus.active:
                    reused_session.status = SessionStatus.revoked
                    reused_session.revoked_at = _now()
                db.commit()
                raise HTTPException(
                    status_code=401,
        ...
        prev = db.scalar(
            ...
            .where(SessionRefreshToken.session_id == session.id)
            .where(SessionRefreshToken.token_hash == session.previous_token_hash)
        )
        if prev and prev.revoked_at is None:
            prev.revoked_at = _now()
        db.add(
            SessionRefreshToken(
    ...
    def logout(db: Session, refresh_token: str):
        token_hash = _hash_token(refresh_token)
        session = db.scalar(
            select(AuthSession).where(AuthSession.token_hash == token_hash).where(AuthSession.revoked_at.is_(None))
        )
        if not session:
...
def request_password_reset(db: Session, email: str) -> dict | None:
    ...
    credential = db.scalar(
        select(UserCredential).where(UserCredential.person_id == person.id).where(UserCredential.is_active.is_(True))
    )
    if not credential:
        return None

    token = _issue_password_reset_token(db, str(person.id), email)
    expires_at = _now() + timedelta(minutes=_password_reset_ttl_minutes(db))
...
def reset_password(db: Session, token: str, new_password: str) -> datetime:
    ...
    reset_record = db.scalar(
        ...
        .where(PasswordResetToken.person_id == person_uuid)
        .where(PasswordResetToken.used_at.is_(None))
    )
    now = _now()
    expires = _as_utc(reset_record.expires_at) if reset_record else None
    if not reset_record or not expires or expires <= now:
        raise HTTPException(status_code=401, detail="Invalid reset token")
