        ...
        for spec in SETTINGS_SPECS:
            ...
            if spec.is_secret and db_raw:
                if isinstance(db_raw, str) and not is_openbao_ref(db_raw):
                    errors.append(f"{spec.domain.value}.{spec.key}: secret must be an OpenBao reference")
                    continue
            ...
            if spec.allowed:
                ...
                if normalized not in {item.lower() for item in spec.allowed}:
                    errors.append(f"{spec.domain.value}.{spec.key}: value must be one of {sorted(spec.allowed)}")
            if spec.min_value is not None:
                try:
                    if int(effective) < spec.min_value:
                        errors.append(f"{spec.domain.value}.{spec.key}: value must be >= {spec.min_value}")
                except (TypeError, ValueError):
                    errors.append(f"{spec.domain.value}.{spec.key}: value must be an integer")
            if spec.max_value is not None:
                try:
                    if int(effective) > spec.max_value:
                        errors.append(f"{spec.domain.value}.{spec.key}: value must be <= {spec.max_value}")
                except (TypeError, ValueError):
                    errors.append(f"{spec.domain.value}.{spec.key}: value must be an integer")
    finally:
        db.close()
