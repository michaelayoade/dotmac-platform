...
def upgrade() -> None:
    ...
    if "signup_requests" not in tables:
        op.create_table(
            "signup_requests",
    ...
    sr_indexes = (
        {idx["name"] for idx in inspector.get_indexes("signup_requests")} if "signup_requests" in tables else set()
    )
    if "ix_signup_requests_email" not in sr_indexes:
        op.create_index(op.f("ix_signup_requests_email"), "signup_requests", ["email"], unique=False)
    if "ix_signup_requests_org_code" not in sr_indexes:
        op.create_index(op.f("ix_signup_requests_org_code"), "signup_requests", ["org_code"], unique=False)
    if "ix_signup_requests_catalog_item_id" not in sr_indexes:
        op.create_index(
            op.f("ix_signup_requests_catalog_item_id"), "signup_requests", ["catalog_item_id"], unique=False
        )
    if "ix_signup_requests_expires_at" not in sr_indexes:
        op.create_index(op.f("ix_signup_requests_expires_at"), "signup_requests", ["expires_at"], unique=False)
    if "ix_signup_requests_verification_token_hash" not in sr_indexes:
        op.create_index(
            op.f("ix_signup_requests_verification_token_hash"),
            "signup_requests",
