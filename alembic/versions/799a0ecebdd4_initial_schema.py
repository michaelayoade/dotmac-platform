...
def upgrade() -> None:
    ...
    if "people" not in tables:
        op.create_table(
            ...
            sa.Column(
                "gender", sa.Enum("unknown", "female", "male", "non_binary", "other", name="gender"), nullable=True
            ),
            sa.Column(
    ...
    mfa_indexes = {idx["name"] for idx in inspector.get_indexes("mfa_methods")} if "mfa_methods" in tables else set()
    if "ix_mfa_methods_primary_per_person" not in mfa_indexes:
        op.create_index(
    ...
    sessions_indexes = {idx["name"] for idx in inspector.get_indexes("sessions")} if "sessions" in tables else set()
    if "ix_sessions_token_hash" not in sessions_indexes:
        op.create_index("ix_sessions_token_hash", "sessions", ["token_hash"])
    if "ix_sessions_previous_token_hash" not in sessions_indexes:
    ...
    if "audit_events" not in tables:
        op.create_table(
            ...
            sa.Column(
                "actor_type", sa.Enum("system", "user", "api_key", "service", name="auditactortype"), nullable=False
            ),
